...
DefineCommandClass(UniIntakeSet, {
    int directionId;
});

static void constructor(va_list argp) {
    self->fields->directionId = va_arg(argp, int);
    String name = UniIntake_getDirectionName(self->fields->directionId);
    ErrorMsgIf(!name, VEXOS_ARGINVALID, "Invalid intake direction: %d", self->fields->directionId);
    setArgs("%s", name);
    require(&UniIntake);
}

static void initialize() {
    UniIntake_setDirection(self->fields->directionId);
}
