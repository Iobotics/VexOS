...
DefineCommandClass(UniLiftSet, {
    int positionId;
});

static void constructor(va_list argp) {
    self->fields->positionId = va_arg(argp, int);
    String name = UniLift_getPositionName(self->fields->positionId);
    ErrorMsgIf(!name, VEXOS_ARGINVALID, "Invalid lift position: %d", self->fields->positionId);
    setArgs("%s", name);
    require(&UniLift);
}

static void initialize() {
    UniLift_setPosition(self->fields->positionId);
}
